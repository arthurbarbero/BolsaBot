...
from BolsaBot_Scrap.Scrap import scrap
...
def start(bot, update):
    ...
    Digite o nome da empresa ou prefixo listado na bolsa de valores para receber as informações referentes a cotação atual""")
...
def cotacao(bot,update): #Apresenta as informações referente a cotação da empresa escolhida
    ...
    if valor == str('ess":'):
        bot.send_message(chat_id=update.message.chat_id, text="Insira um nome de empresa válido, listada na Bolsa de Valores")
    else:
        bot.send_message(chat_id=update.message.chat_id, text=f"O último fechamento desta empresa foi de: R$ {valor} \n\nFonte: ValorEconomico - Atraso de 15 minutos \n\n Insira o nome de outra empresa que deseja saber sua última cotação!")
